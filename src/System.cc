...
namespace ORB_SLAM3 {
    ...
    Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp,
        ...
        std::vector<float> newRow = {mpTracker->mCurrentFrame.visibility, mpTracker->mCurrentFrame.SVE_a,
                                     mpTracker->mCurrentFrame.SVE_b, mpTracker->mCurrentFrame.SVE_c};
...
//
        ...
        cout << "file open" << endl;
        f << fixed;
        f << "Time,t_0,t_1,t_2,q_x,q_y,q_z,q_w" << endl;
...
//            cout << "--Parent KF: " << pKF->mnId << endl;
        ...
        f.close();
        cout << endl << "End of saving trajectory to " << filename << " ..." << endl;
        cout << "Total points saved: " << writtenCount << endl << flush;
    }
...
/* ---------- <SVE> ---------- */
// similar to savekeyframetrajectory, this function saves all the recorded visibility statistics to a .txt file.
    void System::SaveVisibilityStatistics(const string &filename) {
        cout << endl << "Saving visibility statistics to " << filename << " ..." << endl << flush;

        ofstream f;
        f.open(filename.c_str());
        f << fixed;
        f << "Time,SVE,SVE_a,SVE_b,SVE_c" << endl;

        for (size_t i = 0; i < vSVE_t.size(); i++) {
            double timestamp = 1e9 * vSVE_t[i];  // match scale of frame and keyframe timestamps
            float visibility = vSVE[i][0];
            float SVE_a = vSVE[i][1];
            float SVE_b = vSVE[i][2];
            float SVE_c = vSVE[i][3];

            f << setprecision(6) << timestamp << "," << setprecision(6) << visibility << "," << setprecision(6) << SVE_a
              << "," << setprecision(6) << SVE_b << "," << setprecision(6) << SVE_c << endl;

        }

        f.close();
        cout << endl << "SVE stats saved! Total count: " << vSVE_t.size() << endl << flush;
    }
