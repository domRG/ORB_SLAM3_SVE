...
#define IMU_PATH "/imu0"
#define CAM_PATH "/cam0/image_raw"
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber sub_imu = n.subscribe(IMU_PATH, 1000, &ImuGrabber::GrabImu, &imugb);
    ros::Subscriber sub_img0 = n.subscribe(CAM_PATH, 100, &ImageGrabber::GrabImage, &igb);
    ...
    if (!ros::ok()) {
        cout << endl << "Terminated by ctrl+c - DOM" << endl << flush;
        // program terminated, usually by ctrl-c
	    SLAM.Shutdown();
        // save outputs
        const string f_file =  "ros_output_traj.txt";
        const string sve_file = "ros_output_sve.txt";
        SLAM.SaveTrajectory(f_file);
